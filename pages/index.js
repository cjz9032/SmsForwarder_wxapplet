...
function 按钮1_被单击(){
	...
	if(编辑框1.取内容()=="" || 编辑框2.取内容()==""){
		...
		if(是否编辑==true ){
			...
			在线列表1.置项目标记(当前项目,threeAmigo)
			保存主机数据()
			窗口操作.隐藏面板(0,2)
			对话框1.弹出提示2("已保存")
			}else{
			主机=读写设置.读取设置("主机列表")+编辑框1.取内容()+"#"+编辑框2.取内容()+"#"+threeAmigo+"\n"
			读写设置.保存设置("主机列表",主机)
			在线列表1.添加项目("/images/sy.png",编辑框1.取内容(),编辑框2.取内容(),"操作",threeAmigo)
			对话框1.弹出提示2("已添加")
